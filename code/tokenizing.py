...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Get model directory and path to files that need to be rewritten."
    )
    parser.add_argument("--corpus", help="path to corpus directory", required=True)
    parser.add_argument("--new_path", help="provide a new name for the tokenized corpus")
    ...
    if not args.new_path:
        corpus_path_fil = corpus_path + "-tok"
    else:
    ...
    corpora = os.listdir(corpus_path)
    print(corpora)

    # get length of fnames for each subcorpus combined
    all_files = [len(os.listdir(os.path.join(corpus_path, s))) for s in corpora]
    print(sum(all_files))
    exit()

    for sub_corpus in corpora[:1]:
        ...
        for i in tqdm(range(len(fnames)), desc=sub_corpus):
            ...
            with open(os.path.join(corpus_path, sub_corpus, fname), mode="r") as in_file, open(
                os.path.join(corpus_path_fil, sub_corpus, fname), mode="w"
            ) as out_file:
                for line in in_file:
